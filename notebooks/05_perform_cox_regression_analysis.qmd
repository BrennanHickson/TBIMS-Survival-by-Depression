...
# Save the table
gtsave(tbl_nested_models, here(tables_dir, "table_2_nested_cox_regression_models_10_year_followup.png"))
```
...
saveRDS(model_1, here(data_processed_dir, "cox_model_1_10_year_followup.rds"))
saveRDS(model_2, here(data_processed_dir, "cox_model_2_10_year_followup.rds"))
saveRDS(model_3, here(data_processed_dir, "cox_model_3_10_year_followup.rds"))
saveRDS(model_4, here(data_processed_dir, "cox_model_4_10_year_followup.rds"))
saveRDS(model_5, here(data_processed_dir, "cox_model_5_10_year_followup.rds"))
...
vif_table <- tbl_regression(model_5_vif, label = var_name_mapping, pvalue_fun = purrr::partial(style_pvalue, digits = 3)) |>
  modify_header(label ~ "**VIF Summary Table**", p.value ~ "**_p_-value**") |>
  bold_labels() |>
  bold_p() |>
  add_vif() |>
  as_gt()
...
# Save the table
gtsave(vif_table, here(tables_dir, "table_3_vif_summary_table_10_year_followup.png"))
...
plot_file_path <- file.path(multicollinearity_plots_dir, "correlation_matrix_10_year_followup.png")

# Save the correlation matrix plot to a file
png(filename = plot_file_path, width = 4000, height = 3000, res = 300)
...
# Perform the Schoenfeld test on Models 1-5
...
schoenfeld_test_results_model_5 <- cox.zph(model_5, transform = "identity")
...
create_plot <- function(index, label) {
  ...
  plot_file <- file.path(schoenfeld_residuals_plots_dir, paste0("plot_", index, ".png"))
  ...
  plot(schoenfeld_test_results_model_5[index], main = "Proportional Hazards Assumption Test", xlab = "Time from Year 1 Interview (years)", ylab = label, xaxt = "n", family = "Proxima Nova")
...
cat("Plots saved to:", schoenfeld_residuals_plots_dir, "\n")
```
...
gg.calendar_year_of_injury_vs_martingale <- ggplot(martingale_plot_data, aes(x = Calendar_Year, y = Martingale_Residuals)) +
  ...
  geom_smooth(method = "loess", span = 0.9, color = "#EF5350", linewidth = 1.25) + # Add a Loess curve
...
# Fit the survival model for Model 5
...
# Fit a survival model using survfit2 from ggsurvfit
surv_model_5 <- survfit2(Surv(time_to_event_in_years, event_status) ~ depression_level_at_year_1 + sex, data = complete_cases_all)

# Define the color palette with enough values
color_palette <- c("#90cbf9", "#2196f3", "#114b7a", "#f990cb", "#f32196", "#7a114b")

# Generate the Kaplan-Meier survival curve plot with risk table using ggsurvfit
gg.surv.model5 <- ggsurvfit(surv_model_5) +
  labs(
    x = "Time After Initial Interview (years)",
    y = "Survival Probability",
    title = "Effect of Depression Level at Year 1 and Sex on Survival"
  ) +
  scale_color_manual(
    values = color_palette,
    name = "Depression Level at Year 1"
  ) +
  ...
  theme_classic() +
  customization +
  theme(legend.position = c(0.075, 0.20)) +
