...
pacman::p_load(autoReg, broom, corrplot, extrafont, forestplot, gridExtra, gt, gtsummary, here, scales, showtext, survival, survminer, tidyverse)
...
create_summary_table <- function(model, var_name_mapping) {
  tbl <- tbl_regression(model, exponentiate = TRUE) |>
    ...
    # modify_footnote(label ~ "Custom labels applied.") # Add a footnote
...
table_data$Variable <- variable_var_name_mapping[table_data$Variable]

# Handle any variables that may not have a mapping defined
table_data$Variable[is.na(table_data$Variable)] <- rownames(coef)[is.na(variable_var_name_mapping[rownames(coef)])]
