...
pacman::p_load(autoReg, broom, corrplot, extrafont, forestplot, gridExtra, gtsummary, here, scales, showtext, survival, survminer, tidyverse)
...
# Load the .rds files
complete_cases_all <- readRDS(file.path(data_processed_dir, "archived/05-31-2024 (5-Year Follow-Up)/complete_cases_all.rds"))
...
all_proposed_functional_vars <- c("func_score_at_year_1")
...
```{r Define the Preferred Variable Labels for the Covariates, include=FALSE}
# Create a variable name mapping of original variable names to preferred labels
name_mapping <- c(
  ...
  # drs_total_at_year_1 = "DRS Total at Year 1",
  # fim_total_at_year_1 = "FIM Total at Year 1",
  # gose_total_at_year_1 = "GOS-E Total at Year 1",
...
get_var_labels <- function(model, name_mapping) {
  ...
  for (i in seq_along(var_names)) {
    ...
    # Attempt to match each part of the split name with keys in the name_mapping
    label_parts <- sapply(split_name, function(part) {
      # Check if the part directly matches a key in the name_mapping
      if (part %in% names(name_mapping)) {
        return(name_mapping[part])
      } else {
        # If the part is a level of a factor, construct the key to find its label
        possible_keys <- grep(part, names(name_mapping), value = TRUE)
        if (length(possible_keys) > 0) {
          # Return the first match found
          return(name_mapping[possible_keys[1]])
        } else {
...
save_schoenfeld_residuals_plots <- function(model, model_number, name_mapping, plot_width = 7, plot_height = 5.5) {
  ...
  var_labels <- get_var_labels(model, name_mapping)
...
# Convert 'event_status' to numeric
if (!is.numeric(complete_cases_all$event_status)) {
  complete_cases_all$event_status <- as.numeric(as.character(complete_cases_all$event_status))
}
...
```{r Models 2-5: Fit the Multivariable Models for All Proposed Covariates}
...
tbl_model_2 <- tbl_regression(model_2, exponentiate = TRUE, label = name_mapping, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) |>
  # modify_header(label ~ "**Model 2: Mental Health**") |>
  bold_labels() |>
  bold_p()

tbl_model_3 <- tbl_regression(model_3, exponentiate = TRUE, label = name_mapping, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) |>
  # modify_header(label ~ "**Model 3: Mental Health + Sociodemographic**") |>
  bold_labels() |>
  bold_p()

tbl_model_4 <- tbl_regression(model_4, exponentiate = TRUE, label = name_mapping, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) |>
  # modify_header(label ~ "**Model 4: Mental Health + Sociodemographic + Clinical**") |>
  bold_labels() |>
  bold_p()

tbl_model_5 <- tbl_regression(model_5, exponentiate = TRUE, label = name_mapping, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) |>
...
tbl_nested_models <- tbl_merge(
  tbls = list(tbl_model_2, tbl_model_3, tbl_model_4, tbl_model_5),
  tab_spanner = c("**Model 2**", "**Model 3**", "**Model 4**", "**Model 5**")
)

# Convert the merged table
...
create_summary_table <- function(model, name_mapping) {
  tbl <- tbl_regression(model, exponentiate = TRUE) |>
    modify_header(label ~ "**Variable**") |> # Bold the header
    bold_labels() |> # Bold the variable names
    modify_footnote(label ~ "Custom labels applied.") # Optional: add a footnote

  for (old_name in names(name_mapping)) {
    new_name <- name_mapping[[old_name]]
    tbl <- tbl |> 
      update_cols(all_of(old_name), label = new_name) 
  }
  return(tbl)
}

# Summarize the models using tbl_regression from gtsummary and apply custom labels
summary_model_2 <- create_summary_table(model_2, name_mapping)
summary_model_3 <- create_summary_table(model_3, name_mapping)
summary_model_4 <- create_summary_table(model_4, name_mapping)
summary_model_5 <- create_summary_table(model_5, name_mapping)

# Optionally, you can modify and print the tables with gt
...
# Save the Cox proportional hazards model as an R object
...
saveRDS(model_5, here(data_processed_dir, "cox_model_5.rds"))
# saveRDS(model_6, here(data_processed_dir, "cox_model_6.rds"))
```
...
```{r Assessing Multicollinearity via the Variance Inflation Factor}
model_5_vif <- lm(event_status ~ depression_level_at_year_1 + mental_health_tx_hx + psych_hosp_hx + suicide_attempt_hx +
                    calendar_year_of_injury + age_at_injury + sex + education_level_at_injury + employment_at_injury + 
                    marital_status_at_injury + rehab_payor_primary_type + cause_of_injury + problematic_substance_use_at_injury +
                    problematic_substance_use_at_year_1 + func_score_at_year_1,
                  data = complete_cases_all)

# Create a gtsummary table for the model
vif_table <- tbl_regression(model_5_vif, label = name_mapping, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) |>
  modify_header(label ~ "**VIF Summary Table**") |>
  bold_labels() |>
  bold_p() |>
  add_vif() |>  # Add VIF values
  as_gt()  # Convert the gtsummary object to a gt table
...
```{r Assessing Multicollinearity via the Correlation Matrix}
predictors <- complete_cases_all |>
  select(-time_to_event_in_years, -event_status)  # Exclude the survival object variables
...
colnames(correlation_matrix) <- name_mapping[colnames(correlation_matrix)]
rownames(correlation_matrix) <- name_mapping[rownames(correlation_matrix)]
...
```{r}
# Function to format and print p-values and chi-square values
format_and_print_p_values <- function(schoenfeld_test_results) {
  results <- as.data.frame(schoenfeld_test_results$table)
  results <- results %>%
    mutate(chisq = formatC(chisq, format = "f", digits = 3),
           p_value_formatted = ifelse(`p` < 0.05,
...
format_and_print_p_values(schoenfeld_test_results_model_2)
format_and_print_p_values(schoenfeld_test_results_model_3)
format_and_print_p_values(schoenfeld_test_results_model_4)
format_and_print_p_values(schoenfeld_test_results_model_5)
```
...
# Create a data frame of continuous variables and Martingale residuals for plotting
...
gg.calendar_year_of_injury_vs_martingale <- ggplot(martingale_plot_data, aes(x = Calendar_Year, y = Martingale_Residuals)) +
  ...
  scale_y_continuous(breaks = seq(-2.5, 1.0, by = 0.5), limits = c(-2.5, 1.0)) + # Adjust y-axis tick values
...
ggsave(here(martingale_residuals_plots_dir, "figure_19-1_calendar_year_of_injury_vs_martingale_residuals.png"), 
       ...
       width = 10, # Specify the width in inches. Adjust this value as needed.
       height = 6, # Specify the height in inches. Adjust if necessary, or keep it for the desired aspect ratio.
       bg = "#FFFFFF",
       dpi = 300)
```
...
# Relevel the strata factors and set preferred reference levels
complete_cases_all$sex <- relevel(complete_cases_all$sex, ref = "Male")
complete_cases_all <- complete_cases_all |>
  mutate(depression_level_at_year_1 = factor(depression_level_at_year_1,
                                             levels = c("No Depression", "Minor Depression", "Major Depression")))
# Define the strata
strata <- expand.grid(
  depression_level_at_year_1 = levels(complete_cases_all$depression_level_at_year_1),
  sex = levels(complete_cases_all$sex)
  )
# Ensure the length of row names matches the number of rows in strata
rownames(strata) <- letters[1:nrow(strata)]

# Define the stratum manually since there's only one factor
stratum <- data.frame(
  depression_level_at_year_1 = levels(complete_cases_all$depression_level_at_year_1)
)
rownames(stratum) <- letters[1:nrow(stratum)]

# Define the average participant
average_participant <- expand.grid(
  depression_level_at_year_1 = levels(complete_cases_all$depression_level_at_year_1),
  sex = levels(complete_cases_all$sex),
  calendar_year_of_injury = 2009,
  age_at_injury = 34,
  education_level_at_injury = 12,
  ...
  drs_total_at_year_1 = 1.5,
  fim_total_at_year_1 = 121,
  gose_total_at_year_1 = 6,
  func_score_at_year_1 = 0.5795526,
...
# Relevel the strata factors and set preferred reference levels
average_participant$sex <- relevel(average_participant$sex, ref = "Male")

average_participant <- average_participant |>
  mutate(depression_level_at_year_1 = factor(depression_level_at_year_1,
                                             levels = c("No Depression", "Minor Depression", "Major Depression")))
```
...
gg.surv.model1 <- surv_model_1 |>
  ggsurvplot_df(
    ...
    surv.scale = "percent",
    break.time.by = 2, 
    xlim = c(0, 10), # Limit x-axis to 10 years
    ylim = c(crop, 1),
...
ggsave(here(survival_curves_plots_dir, "figure_19-1_kaplan_meier_curves_model_1.png"),
...
gg.surv.model5 <- surv_model_5 |>
  ggsurvplot_df(
    ...
    surv.scale = "percent",
    break.time.by = 1,
    xlim = c(0, 5), # Limit x-axis to 10 years
    ylim = c(crop, 1),
    palette = c("#90cbf9", "#2196f3", "#114b7a"),
    ggtheme = theme_classic(),
  ) +
...
ggsave(here(survival_curves_plots_dir, "figure_19-2_kaplan_meier_curves_model_5.png"),
       gg.surv.model5, dpi = 300)
```

```{r}
# Define the mapping of original variable names to display names
...
table_data$Variable <- variable_name_mapping[table_data$Variable]

# Handle any variables that may not have a mapping defined
table_data$Variable[is.na(table_data$Variable)] <- rownames(coef)[is.na(variable_name_mapping[rownames(coef)])]

# Format the table for the forest plot
table_text <- cbind(
  Variable = table_data$Variable,
  HR = sprintf("%.2f", table_data$HR),
  CI = paste0("(", sprintf("%.2f", table_data$Lower_CI), " - ", sprintf("%.2f", table_data$Upper_CI), ")"),
  P_value = sprintf("%.3f", table_data$P_value)
)
...
forestplot(
  ...
  mean = c(NA, table_data$HR),  # Add NA for the header row
  lower = c(NA, table_data$Lower_CI),
  upper = c(NA, table_data$Upper_CI),
...
```{r Plot Forest Plot for Cox Proportional Hazards Model}
# Generate a ggforest plot
ggforest_5 <- ggforest(model_5,
                       refLabel = "Reference",
                       data = as.data.frame(complete_cases_all))

ggforest_5 + scale_y_discrete(labels = c("depression_level_at_year_1" = "Depression Level at Year 1",
                                         "mental_health_tx_hx" = "History of Mental Health Treatment"))
