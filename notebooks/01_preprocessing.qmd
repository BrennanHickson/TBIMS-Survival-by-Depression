...
baseline_name_and_na_mappings <- list(
  ...
  age = list(original_name = "AGE", na_values = 9999, to_class = "numeric"),
  ...
  suicide_attempt_hx_lifetime_at_injury = list(original_name = "Suicide", na_values = c(66, 77, 88, 99), to_class = "factor"),
  suicide_attempt_hx_past_year_at_injury = list(original_name = "SuicidePrior", na_values = c(66, 77, 88, 99), to_class = "factor")
  )
...
merged_data <- merged_data |>
  ...
  mutate(
    ...
    # suicide_attempt_hx_past_year = coalesce(suicide_attempt_hx_past_year_at_injury, suicide_attempt_hx_past_year_at_followup)
  )
```
...
```{r Define a Function to Manage, Combine, and Update Value Labels}
#' Combine value labels from two variables into a unified label set
#'
#' This function combines value labels from two variables and returns a vector of unique labels.
#'
#' @param var1 A variable with value labels.
#' @param var2 A variable with value labels.
#' @return A vector of unique value labels.
#' @export
#' @examples
#' combine_value_labels(c(1, 2, 3), c(2, 3, 4))
...
#   combined_value_labels <- unique(c(labels1, labels2))
#   return(combined_value_labels)
# }
...
# # Combine and update the value labels for the coalesced 'problematic_substance_use' variable
# combined_value_labels <- combine_value_labels(merged_data$problematic_substance_use.x, merged_data$problematic_substance_use.y)
# merged_data$problematic_substance_use <- set_labels(merged_data$problematic_substance_use, labels = combined_value_labels)
...
# Perform additional calculations, collapse factor levels, and convert data types
  mutate(
    ...
    age = if_else(is.na(age), floor(as.duration(interval(date_of_birth, date_of_injury))/dyears(1)), age),
    ...
    employment_at_injury = employment_at_injury |>
      ...
      fct_collapse(
        ...
        Other = c("Special Employed", "Homemaker", "Retired", "Volunteer", "Other")
      ) |>
      fct_relevel("Competitively Employed", "Unemployed", "Student", "Other"),
    ...
    # Set 'No' as the reference group for the lifetime hx of suicide attempt at time of injury covariate
    ...
    # # Set 'No' as the reference group for the past-year hx of suicide attempt covariate
    # suicide_attempt_hx_past_year = fct_recode(suicide_attempt_hx_past_year,
    #                                           No = "0",
    #                                           Yes = "1"
    #                                           ) |>
    #   fct_relevel("No", "Yes")
...
baseline_vars <- c(
  ...
  "employment_at_injury", "rehab_payor_primary",
...
merged_data <- merged_data |>
  mutate(
    across(all_of(baseline_vars), ~ if (is.factor(.)) { as.character(.) } else { . }) 
  )
...
The following code chunk computes two new variables, which are added to the merged data frame: `time_to_expiration` and `time_to_censorship`. The calculations for these variables are based on the dates of injury, follow-up, and death for each observation. Consideration is given to whether or not each observation is the last observation for a given `id`; if it is, then the `time_to_expiration` calculation is prioritized (assuming a valid date is entered for `date_of_death`); if additional observations exist for the current participant, then the `time_to_censorship` calculation is prioritized (assuming a valid date is entered for `date_of_followup`). The `dplyr` and `lubridate` packages facilitate data manipulation and date calculations, respectively. The result is a modified data frame that includes the two new variables.

```{r Calculate Time to Censorship and Time to Expiration, include=FALSE}
# merged_data <- merged_data |>
#   arrange(id, date_of_injury, date_of_followup, date_of_death) |>
#   group_by(id) |>
#   mutate(
#     has_subsequent_observation = !is.na(lead(date_of_injury)),  # Add a column to check for subsequent observations
#     time_to_expiration = case_when(
#       !is.na(date_of_death) & (is.na(date_of_followup) | has_subsequent_observation) ~ as.integer(difftime(date_of_death, date_of_injury, units = "days")),
#       TRUE ~ NA_integer_
#     ),
#     time_to_censorship = case_when(
#       !is.na(date_of_followup) & !has_subsequent_observation ~ as.integer(difftime(date_of_followup, date_of_injury, units = "days")),
#       TRUE ~ NA_integer_
#     )
#   ) |>
#   ungroup() |>
#   select(-has_subsequent_observation)
```
...
```{r Define a Function to Extract the Year 1 Variables of Interest from the Follow-Up Data and Impute the Values Forward and Backward, include=FALSE}
...
#' 
#' @examples
#' # Assuming `my_dataset` is your dataset
#' result <- extract_and_impute_year_1_data(my_dataset)
#'
#' @export
# extract_and_impute_year_1_data <- function(data) {
#   # Extract and rename the Year 1 data for the new variables
#   year_1_data <- data |>
...
#     rename(drs_total_at_year_1 = drs_total_at_followup,
#            fim_total_at_year_1 = fim_total_at_followup,
#            gose_total_at_year_1 = gose_total_at_followup,
#            problematic_substance_use_at_year_1 = problematic_substance_use_at_followup,
#            suicide_attempt_hx_past_year_at_year_1 = suicide_attempt_hx_past_year_at_followup)
# 
#   # Rejoin the Year 1 data with the original dataset
#   data_with_year_1_vars <- data |>
#     left_join(year_1_data, by = "id")
# 
#   # Define the variables to be imputed/filled
#   year_1_vars_to_impute <- c("drs_total_at_year_1",
#                              "fim_total_at_year_1",
...
#                              "positive_symptoms_at_year_1",
#                              "depression_level_at_year_1")
#   # Impute the values for each participant
#   imputed_data <- data_with_year_1_vars |>
#     group_by(id) |>
...
# }
# 
# # Extract and impute the Year 1 problematic substance use and past-year suicide attempt data for the analytic sample
# merged_data <- extract_and_impute_year_1_data(merged_data)
```
...
```{r Define a Helper Function to Verify Whether a Specified Date Falls within the Study Period}
# Define a function to verify whether a specified date falls within the study period
...
**Filtering by Date**
First, the function retains data from participants whose dates of injury fell within the start and end dates of the observation period. 
**Grouping and Flagging**
...
**Inclusion Criteria**
The function uses a combination of these flags and the `data_collection_period` to determine whether an observation should be included in the returned data frame. The inclusion is based on several factors:
...
**Event Status and Time to Event**
This section of the function calculates two important pieces of information for the survival analysis:
1. **Event Status (`event_status`):**
  - This is a binary indicator (0 or 1) indicating whether a significant event (i.e., mortality) occurred prior to the study end date.
  - This variable is calculated using `if_else(!is.na(date_of_death) & date_of_death <= study_end_date, 1, 0)`, where `event_status` is set to 1 (indicating that the event occurred) if `date_of_death` is not missing (`!is.na(date_of_death)`) and occurred on or before the study end date; otherwise, it is set to 0.
2. **Time to Event (`time_to_event`):**
  - This represents the time (in days) from the date of injury to the event or the last valid follow-up date.
  - This variable is calculated differently for baseline and follow-up observations:
    - For baseline (`data_collection_period == 0`), the time to event is set to 0.
    - For follow-up observations, `time_to_event` is the number of days between the date of injury and the later of either the follow-up date or the date of death, computed using `case_when(...)`. The `pmax(date_of_followup, date_of_death, na.rm = TRUE)` function finds the latest of these two dates within a given observation, and `as.numeric(last_valid_date - date_of_injury, units = "days")` calculates the time difference in days.
3. **`time_to_censorship` and `time_to_expiration`:**
...
```{r Define a Function to Apply Study Eligibility Criterion 1 (Inclusion): Injured between October 1, 2006 and October 1, 2012}
...
#' @param observation_period_start_date The start date of the observation period (inclusive).
#' @param observation_period_end_date The end date of the observation period (inclusive).
...
#' @export
apply_criterion_1 <- function(data, observation_period_start_date, observation_period_end_date) {
  ...
  data <- data |>
    ...
    mutate(
      ...
      include_observation = if_else(data_collection_period == 0, TRUE, valid_followup | (special_case & lead(valid_followup, default = TRUE)))
    ) |>
    ...
    mutate(
      ...
      time_to_censorship = if_else(is.na(date_of_death) & is.na(date_of_followup), 0, 
                                   if_else(!is.na(date_of_followup), as.numeric(date_of_followup - date_of_injury, units = "days"), NA_real_)),
      time_to_expiration = if_else(is.na(date_of_death) & is.na(date_of_followup), 0, 
                                   if_else(!is.na(date_of_death), as.numeric(date_of_death - date_of_injury, units = "days"), NA_real_)),
...
#'
#' This function applies the second study eligibility criterion, which involves
#' excluding participants based on early mortality and negative survival times.
#'
#' @param data A data frame containing the study data.
#' @return A data frame after applying Criterion 2, with excluded observations removed.
#' @details
#' The function performs the following operations:
#' 1. Filters out observations with missing or negative 'time_to_event' values, unless the observation
#'    is not the last one for the current 'id' and 'time_to_event' is NA.
#' 2. Logs the sample size and IDs of participants removed after applying this criterion.
#' 3. Returns the modified dataset.
#' @examples
#' # Apply Criterion 2 to a dataset
#' modified_data <- apply_criterion_2(dataset)
#' @export
...
# Apply Study Eligibility Criterion 1
analytic_data <- apply_criterion_1(merged_data, observation_period_start_date, observation_period_end_date)
...
sampled_analytic_data <- analytic_data |>
  ...
  select(-date_of_birth, -age, -status_at_followup) |>
  arrange(id, data_collection_period)
...
```{r Define Functions to Calculate Depression Levels at Year 1 and Impute the Resulting Data to Previous and Subsequent Observations, include=FALSE}
#' Calculate depression level at Year 1
#'
#' This function calculates the depression level at Year 1 based on participants' PHQ-9 scores.
#'
#' @param data A data frame.
#' @return A data frame with depression level calculated.
#' @export
#' @examples
#' calculate_depression_level(data)
# calculate_depression_level <- function(data) {
#   # Create a unique identifier for each observation
#   data <- data |> mutate(unique_id = paste(id, data_collection_period, sep = "_"))
#   
#   # Filter and perform calculations for Year 1
#   year_1_data <- data |>
#     filter(data_collection_period == 1) |>
#     rowwise() |> 
#     mutate(
#       positive_symptoms_at_year_1 = sum(phq1 >= 1, phq2 >= 1, phq3 >= 1, phq4 >= 1, phq5 >= 1, phq6 >= 1, phq7 >= 1, phq8 >= 1, phq9 >= 1),
#       cardinal_symptoms_at_year_1 = case_when(
...
#         TRUE ~ NA_character_
#       ),
#       depression_level_at_year_1 = case_when(
...
#     ungroup() |>
#     mutate(
#       cardinal_symptoms_at_year_1 = factor(cardinal_symptoms_at_year_1, levels = c("0", "1", "2", "3"), labels = c("None", "Anhedonia", "Depressed Mood", "Both")),
#       depression_level_at_year_1 = factor(depression_level_at_year_1, levels = c("0", "1", "2"), labels = c("No Depression", "Minor Depression", "Major Depression"))
#     ) |> 
#     select(unique_id, cardinal_symptoms_at_year_1, positive_symptoms_at_year_1, depression_level_at_year_1)
#   
#   # Merge the Year 1 data back to the original data, aligning with Year 1 observations
#   data <- data |>
#     left_join(year_1_data, by = "unique_id") |>
#     mutate(
#       cardinal_symptoms_at_year_1 = if_else(data_collection_period == 1, cardinal_symptoms_at_year_1, factor(NA, levels = c("0", "1", "2", "3"), labels = c("None", "Anhedonia", "Depressed Mood", "Both"))),
#       positive_symptoms_at_year_1 = if_else(data_collection_period == 1, positive_symptoms_at_year_1, NA_real_),
#       depression_level_at_year_1 = if_else(data_collection_period == 1, depression_level_at_year_1, factor(NA, levels = c("0", "1", "2"), labels = c("No Depression", "Minor Depression", "Major Depression")))
#     ) |>
#     select(-unique_id)  # Remove the temporary identifier
# 
#   return(data)
# }
# 
# # Calculate the Year 1 depression levels for the analytic sample
# analytic_data <- calculate_depression_level(analytic_data)
```
...
#' @export
# extract_and_impute_year_1_data <- function(data) {
#   # Initialize new variables only if absent from the data frame
#   new_vars <- c("drs_total_at_year_1", "fim_total_at_year_1", "gose_total_at_year_1",
#                 "problematic_substance_use_at_year_1", "suicide_attempt_hx_past_year_at_year_1",
#                 "cardinal_symptoms_at_year_1", "positive_symptoms_at_year_1", "depression_level_at_year_1")
# 
#   for (var in new_vars) {
...
#     left_join(year_1_data, by = "id")
#   
#   # # Identify columns with "_y1" suffix and their corresponding original columns
#   # y1_cols <- names(data_with_year_1_vars)[grepl("_y1$", names(data_with_year_1_vars))]
#   # original_cols <- sub("_y1$", "", y1_cols)
#   # 
#   # # Update original columns with Year 1 data and remove "_y1" columns
#   # for (i in seq_along(y1_cols)) {
#   #   data_with_year_1_vars <- data_with_year_1_vars |>
#   #     mutate(!!original_cols[i] := coalesce(!!sym(y1_cols[i]), !!sym(original_cols[i]))) |>
#   #     select(-!!sym(y1_cols[i]))
#   # }
# 
#   # Define the variables to be imputed/filled
#   year_1_vars_to_impute <- c("drs_total_at_year_1",
...
# 
# # Extract and impute the Year 1 problematic substance use and past-year suicide attempt data for the analytic sample
# analytic_data_depression_imputed <- extract_and_impute_year_1_data(analytic_data_depression)
# 
# # Save the 'analytic_data' data frame in a single .rds file
# saveRDS(analytic_data, here(data_processed_dir, "analytic_data.rds"))
# 
# # Save the 'analytic_data' data frame as a CSV file in the 'data/processed' subdirectory
# write.csv(analytic_data, file.path(data_processed_dir, "analytic_data.csv"), row.names = FALSE)
```
...
```{r Select and Arrange the Final Data Columns in the Final Analytic Sample Data Frame}
# Select and reorder merged data columns
analytic_data_imputed <- analytic_data_imputed |>
  select(
    id, data_collection_period,
    status_at_followup, event_status, 
    time_to_event, time_to_censorship,
    time_to_expiration, date_of_followup,
    date_of_death, date_of_birth,
    date_of_injury, sex, age, cause_of_injury,
    marital_status_at_injury,
    education_level_at_injury,
    employment_at_injury,
    rehab_payor_primary,
...
```{r Retain Only Final Observation Per Participant}
# Retain only the last observation for each unique 'id' based on the maximum 'time_to_event'
...
# Write the sampled data frame to a CSV file
write.csv(sampled_analytic_data_final, file = "~/Downloads/sampled_analytic_data_final.csv", row.names = FALSE)
```