...
merged_data <- merged_data |>
  # Create and impute quartiles for func_score_at_year_1_q5
  group_by(id) |>
  mutate(
...
# Perform additional calculations, collapse factor levels, and convert data types
  mutate(
    ...
    # Convert 'func_score_at_year_1_q5' to a factor
    func_score_at_year_1_q5 = as.factor(func_score_at_year_1_q5),
    # Set 'Q5' as the reference group for the function factor score at year 1 quintiles covariate
    func_score_at_year_1_q5 = fct_recode(func_score_at_year_1_q5,
                                         Q1 = "1",
                                         Q2 = "2",
                                         Q3 = "3",
                                         Q4 = "4",
...
#' Log Sample Size
#'
#' This function logs the number of unique IDs and total observations in a dataset after applying a specified criterion.
#' If an original dataset is provided, it also logs the number of unique participants in the original dataset.
#'
#' @param data A data frame containing the dataset after applying the criterion.
#' @param criterion_number An integer specifying the criterion number applied to the data.
#' @param base_directory A character string specifying the base directory where the log file will be stored. Defaults to "logs".
#' @param original_data An optional data frame containing the original dataset before applying any criterion. Defaults to NULL.
#'
#' @details This function ensures that the base directory exists and then creates a log file named "sample_sizes.log" in the specified directory.
#' It calculates the number of unique IDs and total observations in the provided dataset. If the original dataset is provided,
#' it also calculates the number of unique participants in the original dataset. The function then formats these numbers with commas
#' and constructs a log message that is appended to the log file.
#'
#' @return This function does not return any value. It writes the log message to a file.
#'
#' @examples
#' \dontrun{
#' # Example usage
#' data <- data.frame(id = c(1, 2, 2, 3, 3, 3))
#' original_data <- data.frame(id = c(1, 2, 3, 4, 5))
#' log_sample_size(data, criterion_number = 1, original_data = original_data)
#' }
#'
#' @export
...
is_within_study_period <- function(date_of_year_1_followup, date_of_followup) {
  ...
  # participant_end_date <- date_of_year_1_followup + years(10)
...
```{r Define a Function to Apply Study Eligibility Criterion 1 (Inclusion): Injured between October 1, 2006 and October 1, 2012 with Follow-Up for Up to 5 or 10 Years Post-Injury}
...
write.csv(sampled_analytic_data, file = "~/Downloads/5_year_followup/sampled_analytic_data.csv", row.names = FALSE)
# write.csv(sampled_analytic_data, file = "~/Downloads/10_year_followup/sampled_analytic_data.csv", row.names = FALSE)
```
...
saveRDS(analytic_data_imputed, here(data_processed_dir, "/5_year_followup/analytic_data_imputed.rds"))
# saveRDS(analytic_data_imputed, here(data_processed_dir, "/10_year_followup/analytic_data_imputed.rds"))

# Save the 'analytic_data_imputed' data frame as a CSV file in the 'data/processed' subdirectory
write.csv(analytic_data_imputed, file.path(data_processed_dir, "/5_year_followup/analytic_data_imputed.csv"), row.names = FALSE)
# write.csv(analytic_data_imputed, file.path(data_processed_dir, "/10_year_followup/analytic_data_imputed.csv"), row.names = FALSE)
```
  ...
  mutate(
    ...
    suicide_attempt_hx = factor(suicide_attempt_hx, 
                                levels = c("Denied any history of suicide attempt", 
                                           "Suicide attempt history prior to injury", 
                                           "Suicide attempt in the first year post-injury")),
    func_score_at_year_1_q5 = factor(func_score_at_year_1_q5,
                                     levels = c("Q5",
                                                "Q4",
                                                "Q3",
                                                "Q2",
...
saveRDS(analytic_data_final, here(data_processed_dir, "/5_year_followup/analytic_data_final.rds"))
# saveRDS(analytic_data_final, here(data_processed_dir, "/10_year_followup/analytic_data_final.rds"))

# Save the 'analytic_data_final' data frame as a CSV file in the 'data/processed' subdirectory
write.csv(analytic_data_final, file.path(data_processed_dir, "/5_year_followup/analytic_data_final.csv"), row.names = FALSE)
# write.csv(analytic_data_final, file.path(data_processed_dir, "/10_year_followup/analytic_data_final.csv"), row.names = FALSE)
```
...
write.csv(sampled_analytic_data_final, file = "~/Downloads/5_year_followup/sampled_analytic_data_final.csv", row.names = FALSE)
# write.csv(sampled_analytic_data_final, file = "~/Downloads/10_year_followup/sampled_analytic_data_final.csv", row.names = FALSE)
```
...
# Save the 'analytic_data_final' data frame in a single .rds file
saveRDS(analytic_data_final_long, here(data_processed_dir, "/5_year_followup/analytic_data_final_long.rds"))
# saveRDS(analytic_data_final_long, here(data_processed_dir, "/10_year_followup/analytic_data_final_long.rds"))

# Save the 'analytic_data_final' data frame as a CSV file in the 'data/processed' subdirectory
write.csv(analytic_data_final_long, file.path(data_processed_dir, "/5_year_followup/analytic_data_final_long.csv"), row.names = FALSE)
# write.csv(analytic_data_final_long, file.path(data_processed_dir, "/10_year_followup/analytic_data_final_long.csv"), row.names = FALSE)
```
