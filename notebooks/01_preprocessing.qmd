...
# Perform additional calculations, collapse factor levels, and convert data types
  mutate(
    ...
    # # Set 'No' as the reference group for the past-year hx of psychiatric hospitalization at follow-up covariate
    # psych_hosp_hx_past_year_at_followup = fct_recode(psych_hosp_hx_past_year_at_followup,
    #                                                No = "1",
    #                                                Yes = "2",
    #                                                Refused = "7"
    #                                                ) |>
...
```{r Define a Helper Function to Dynamically Calculate Each Participant's End Date}
...
is_within_study_period <- function(injury_date, date) {
  study_end_date <- injury_date + years(10)
  !is.na(date) & date >= observation_period_start_date & date <= study_end_date
}
```
...
apply_criterion_1 <- function(data, observation_period_start_date, observation_period_end_date) {
  ...
  data <- data |>
    ...
    group_by(id) |>
    mutate(
      study_end_date = date_of_injury + years(10),
      valid_followup = is_within_study_period(date_of_injury, date_of_followup) | is_within_study_period(date_of_injury, date_of_death),
    ...
    select(id, data_collection_period, status_at_followup, event_status,
           ...
           calendar_year_of_event, date_of_followup, date_of_death, study_end_date, everything()) |>
